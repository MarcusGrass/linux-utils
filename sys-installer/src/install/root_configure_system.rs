...
pub(crate) async fn root_configure_system(path: &CfgPath) -> Result<()> {
    ...
    .await?;
    show_message_then_increment(
        "Enable docker".to_owned(),
        run_command("systemctl", &["enable", "docker.service"]),
        &pb,
    )
    .await?;
    show_message_then_increment(
    ...
    .await?;
    show_message_then_increment(
        "Add user to wheel".to_owned(),
        run_command("usermod", &["-aG", "wheel", &cfg.user_name]),
        &pb,
    )
    .await?;
    show_message_then_increment(
        "Add user to docker".to_owned(),
        run_command("usermod", &["-aG", "docker", &cfg.user_name]),
        &pb,
    )
    .await?;
